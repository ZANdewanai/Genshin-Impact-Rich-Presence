...
try:
    ...
    GAME_RESOLUTION = user32.GetSystemMetrics(1)  # SM_CYSCREEN = 1 (screen height)
    print(f"Auto-detected screen resolution: {user32.GetSystemMetrics(0)}x{GAME_RESOLUTION}")
except:
    GAME_RESOLUTION = 1080  # Fallback
    print("Could not auto-detect resolution, using fallback 1080p")

"""
The resolution you're running genshin at (number corresponds to the
resolution height in pixels).

NOTE: This is now auto-detected from your screen resolution. If you're using
DLDSR/DLSS/NVIDIA Image Sharpening or any other GPU configuration that performs
image upscaling or oversampling (not counting the built-in AMD FSR2 anti-aliasing mode),
you'll need to manually set this to the final output resolution that your screen displays.
E.g. 75% resolution with NVIDIA Image Sharpening will still result in an image with the
same resolution as your monitor, so you should use the monitor resolution instead.
...
# Domain name coordinates - will be set after scaling logic
# Bounding box coordinates for selected domain name.
#
# (top left X, top left Y, bottom right X, bottom right Y)

GAME_MENU_COORD = (0, 20, 500, 110)
"""
Bounding box for "Game Menu" text.
...
SLEEP_PER_ITERATION = 0.08
"""
How many seconds to sleep per iteration.

Decreased for faster location detection - now ~80ms per loop with 2 OCR attempts.
...
(NOTE: This doesn't need to be too high, as incorrect names will be checked against the database)
Lowered for faster detection - database lookup will filter out bad matches.
"""
INACTIVE_COOLDOWN = 5
"""
...
DEBUG_LOCATION = True
"""
Set to True to enable location OCR debugging (debug images, detailed logging).
"""

DEBUG_CHARACTER = False
"""
Set to True to enable character name OCR debugging.
"""

DEBUG_BOSS = False
"""
Set to True to enable boss OCR debugging.
"""

DEBUG_DOMAIN = False
"""
Set to True to enable domain OCR debugging.
"""

DEBUG_GAME_MENU = False
"""
Set to True to enable game menu OCR debugging.
"""

# Debug Image Saving Settings
DEBUG_SAVE_IMAGES = True
"""
Set to True to save debug images of OCR preprocessing steps to debug_ocr folder.
"""

DEBUG_IMAGES_DIRECTORY = "debug_ocr"
"""
Directory to save debug images when DEBUG_SAVE_IMAGES is True.
"""

# OCR Overlay Settings
ENABLE_OCR_OVERLAY = True  # Enabled for debugging
"""
Set to True to enable visual overlay showing OCR regions with yellow borders and labels.
Useful for debugging coordinate accuracy. Requires tkinter (standard library).
"""

OVERLAY_OPACITY = 0.8
"""
Opacity of the overlay window (0.0 to 1.0). Lower values make it less intrusive.
"""

OVERLAY_TEXT_COLOR = "white"
"""
Color of the text labels on the overlay (e.g., 'white', 'yellow', 'black').
"""

OVERLAY_BORDER_COLOR = "yellow"
"""
Color of the border rectangles (e.g., 'yellow', 'red', 'blue').
"""
...
# OCR Region Labels for Overlay
OCR_REGION_LABELS = {
    "character_numbers": "Active Character",
...
if GAME_RESOLUTION != 0:
    ...
    GAME_MENU_COORD = tuple(round(px * GAME_RESOLUTION / 1440) for px in GAME_MENU_COORD)

# Set domain coordinates - using exact pixel values without scaling
# Format: (left, top, right, bottom)
DOMAIN_COORD = (1260, 130, 1820, 210)  # Using exact pixel values, no scaling
