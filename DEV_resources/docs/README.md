...
### Debugging: Capture OCR regions

Run [capture_ocr_regions.py](capture_ocr_regions.py) to capture images of all screen regions used for OCR:

```bat
py capture_ocr_regions.py
```

This creates a `debug_images/` folder with screenshots showing exactly what areas the OCR is trying to read. Check the [debug_images/README.md](debug_images/README.md) for details on each region.

### Debugging: OCR Post-Processing Pipeline

Run [debug_postprocessing.py](debug_postprocessing.py) to see the complete image processing pipeline:

```bat
py debug_postprocessing.py
```

This creates a `debug_postprocessing/` folder showing all processing stages:
- Raw capture → Grayscale → Blur → Threshold → Morphology
- Compare OCR results at each stage to see what works best

### Debugging: Live OCR Input Images

The main script automatically saves images that Tesseract actually receives to `debug_ocr/` folder during runtime. Check these to see exactly what OCR attempts are processing.
